...
NAME := parsekit
PKG := github.com/rdeusser/parsekit
...
.PHONY: test
test: ## Runs all parsekit's unit tests. This excludes tests in ./test/e2e.
	@echo "==> Running unit tests (without /test/e2e)"
	@go test -v -coverprofile=coverage.out $(shell go list ./... | grep -v /test/e2e);

.PHONY: test/e2e
test/e2e: ## Runs all parsekit's e2e tests from test/e2e.
...
.PHONY: build
build: ## Build parsekit.
	@echo "==> Building parsekit"
	@-CGO_ENABLED=0 \
		go build \
		-o bin/parsekit \
		$(BUILD_PATH)
...
.PHONY: install
install: build ## Install parsekit.
	@echo "==> Installing parsekit"
	@mv ./bin/parsekit $(GOBIN)/parsekit
